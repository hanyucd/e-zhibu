// pages/ask-point-detail/ask-point-detail.js
...
Page({
  ...
  onLoad: function (options) {
    ...
    if (options.type) {
      ...
      page.data.type = options.type;
      db.collection('ask_point_config_list').where({
        _id: page.data.type
      }).get().then(res => {
        if (res.data[0].options){
          page.setData({
            options_list: res.data[0].options
          });
        }
        wx.hideLoading();
      }).catch(err => {
  ...
  onClickDoneBtn(e) {
    wx.showModal({
      title: '提交失败',
      content: '该项目限制还不支持申请积分,过段时间再来看看吧',
      showCancel: false
    });
    return;
    var page = this;
  ...
  onClickCheckbox(e){
    ...
    var tempList = this.data.options_list;
    tempList[index].checked = check;
    this.setData({ 
      options_list: tempList
    })
  },
